...
<div class="conv-page">
  ...
  {:else}
    ...
    {#if conversation.tags.length > 0 || conversation.topics.length > 0}
      <div class="tags-row">
        {#each conversation.topics as topic (topic)}<ComicBadge color="purple" size="sm">{topic}</ComicBadge>{/each}
        {#each conversation.tags as tag (tag)}<ComicBadge color="blue" size="sm">{tag}</ComicBadge>{/each}
      </div>
    {/if}
...
<style>
  .conv-page { display: flex; flex-direction: column; gap: var(--spacing-lg); }
  .conv-header h1 { margin: 0 0 var(--spacing-xs); }
  .meta { display: flex; flex-wrap: wrap; gap: var(--spacing-sm); font-size: 0.8rem; color: var(--text-muted); align-items: center; }
  .section-title { font-size: 0.875rem; text-transform: uppercase; margin: 0 0 var(--spacing-sm); }
  .summary { font-size: 0.875rem; color: var(--text-secondary); margin: 0; }
  .tags-row { display: flex; flex-wrap: wrap; gap: 4px; }
  .messages { display: flex; flex-direction: column; gap: var(--spacing-md); }
  .message { padding: var(--spacing-md); border: var(--border-width) solid var(--border-color); border-radius: var(--radius-sketch); font-size: 0.875rem; line-height: 1.6; white-space: pre-wrap; word-break: break-word; }
  .message.user { background: rgba(0, 210, 106, 0.08); margin-left: 10%; }
  .message.assistant { background: rgba(78, 205, 196, 0.08); margin-right: 10%; }
  .msg-role { font-size: 0.625rem; font-weight: 700; text-transform: uppercase; color: var(--text-muted); margin-bottom: 4px; }
