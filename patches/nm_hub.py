...
Patched for MyTrend Docker deployment: allows Docker bridge network IPs
in addition to localhost. Controlled by NEURAL_MEMORY_HUB_TRUSTED_HOSTS env var.
"""
...
def _build_allowed_hosts() -> set[str]:
    """Build set of allowed hosts from defaults + env var."""
    allowed = {"127.0.0.1", "::1", "localhost"}
    extra = os.environ.get("NEURAL_MEMORY_HUB_TRUSTED_HOSTS", "")
    if extra:
...
async def _require_local_request(request: Request) -> None:
    """Reject requests not from allowed hosts."""
    client_host = request.client.host if request.client else ""
    if client_host not in _ALLOWED_HOSTS:
        logger.warning("Hub request rejected from %s (allowed: %s)", client_host, _ALLOWED_HOSTS)
